...
- [x] **Step 4: Verify public and Admin Calendar DOM tests, TypeScript, ESLint, and the Admin route manually.** Authenticated browser smoke passed against an isolated local backend/frontend on `Month`, `Week`, and `Day`; public smoke passed for `th`, `en`, and `de` at mobile and desktop widths.
