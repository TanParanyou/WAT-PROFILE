...
export default function GalleryListPage() {
    const { data, pagination, sort, isLoading, onPageChange, onSort, fetchData } =
        useDataTable<Gallery>({ fetcher: (p) => galleryAdminService.getAll({ page: p.page, limit: p.limit }) });
    const { confirm, ConfirmDialog } = useConfirm();
    const { toasts, toast, removeToast } = useToast();

    const handleDelete = async (id: number) => {
        if (await confirm({ title: 'ลบรูปภาพ', message: 'ยืนยันการลบรูปภาพนี้?', variant: 'danger' })) {
            try { await galleryAdminService.delete(id); toast.success('ลบสำเร็จ'); fetchData(); } catch { toast.error('ลบไม่สำเร็จ'); }
        }
    };
    ...
    const columns: Column<Gallery>[] = [
        ...
        { header: 'คำอธิบาย (TH)', accessorKey: 'caption', cell: (v) => v?.th || '-', sortable: true },
    ...
    return (
        <div>
            <AdminPageHeader
                title="จัดการคลังภาพ"
                breadcrumbs={[{ label: 'คลังภาพ' }]}
                actions={
                    <div className="flex gap-2">
                        <PermissionButton resource="gallery" action="create" variant="outline" icon={<FolderOpen size={16} />}>
                            <Link href="/admin/gallery/categories">จัดการหมวดหมู่</Link>
                        </PermissionButton>
                        <PermissionButton resource="gallery" action="create" icon={<Plus size={16} />}>
                            <Link href="/admin/gallery/upload">อัปโหลดรูปภาพ</Link>
                        </PermissionButton>
                    </div>
                }
            />
