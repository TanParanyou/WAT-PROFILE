...
export function DayGridView({ controller, entries, resources, onEntryActivate }: DayGridViewProps) {
    ...
    <div className="overflow-x-auto border border-current/15">
      {[...lanes].map(([resourceId, laneEntries]) => (
        <div key={resourceId} className="grid min-h-20 grid-cols-[minmax(9rem,20%)_1fr] border-b border-current/15 last:border-b-0">
          ...
          <div className="space-y-1 p-2">{laneEntries.map((entry) => <CalendarEntryButton key={entry.id} entry={entry} onActivate={onEntryActivate} />)}</div>
        </div>
      ))}
    </div>
  );
