...
export function WeekView({ controller, entries, labels, onEntryActivate }: WeekViewProps) {
        ...
        {days.map((day, index) => {
          ...
          return (
            <div key={key} className="min-w-0">
              <button type="button" onClick={() => controller.selectDate(day)} className="min-h-11 w-full border-b border-current/15 pb-2 text-left text-sm font-semibold focus-visible:outline-2 focus-visible:outline-admin-focus">
                {labels.dayNames[index % 7] ?? ""} {day.getDate()}
              </button>
              <div className="min-h-14 py-1">{allDay.map((entry) => <CalendarEntryButton key={entry.id} entry={entry} onActivate={onEntryActivate} compact />)}</div>
            </div>
          );
        })}
      </div>
      <div className="hidden md:grid md:grid-cols-7 md:gap-3">
        {days.map((day) => <TimeGrid key={formatCalendarDate(day)} day={formatCalendarDate(day)} entries={entries} onEntryActivate={onEntryActivate} showHeader={false} />)}
      </div>
      <div className="md:hidden">
        <TimeGrid day={selectedDay} entries={entries} onEntryActivate={onEntryActivate} />
      </div>
    </div>
  );
}
