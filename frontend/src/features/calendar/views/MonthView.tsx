...
export function MonthView({ controller, entries, labels, onEntryActivate }: MonthViewProps) {
  ...
  return (
    <div className="space-y-4">
          ...
          return (
            <div key={key} className={`min-h-28 border-b border-r border-current/15 p-2 ${isSameDay(day, controller.selectedDate) ? "bg-current/5" : ""}`}>
              <button
                ...
                className="mb-2 min-h-11 min-w-11 px-2 text-left text-sm font-semibold focus-visible:outline-2 focus-visible:outline-admin-focus"
              >
                <span className="block text-[0.7rem] font-normal opacity-60">{labels.dayNames[index % 7] ?? ""}</span>
                {day.getDate()}
              </button>
              <div className="space-y-1">
                {visibleEntries.map((entry) => <CalendarEntryButton key={entry.id} entry={entry} onActivate={onEntryActivate} compact />)}
      ...
      <div className="sm:hidden space-y-2">
        ...
        {entriesOnDay(entries, selectedDay).map((entry) => <CalendarEntryButton key={entry.id} entry={entry} onActivate={onEntryActivate} />)}
