...
export function calendarEntryToneClass(
  ...
  if (variant === "public") {
    if (tone === "warning") return "border-l-4 border-l-site-danger bg-site-surface pl-3 pr-2";
    if (tone === "muted") return "border-l-4 border-l-site-border bg-site-surface text-site-muted pl-3 pr-2";
    return "border-l-4 border-l-site-accent bg-site-surface pl-3 pr-2";
  }

  if (tone === "warning") return "border-l-4 border-l-admin-warning bg-admin-surface-muted pl-3 pr-2";
  if (tone === "muted") return "border-l-4 border-l-admin-border bg-admin-surface-muted pl-3 pr-2";
  return "border-l-4 border-l-admin-info bg-admin-surface-muted pl-3 pr-2";
}
