...
import type { EventRegistrationListItem, RegistrationLocale } from "../types";
...
export function AccountRegistrationsContent() {
  ...
  return <div className="space-y-4">
    ...
    {items.map((item) => <RegistrationCard key={item.id} item={item} locale={locale} editing={editingId === item.id} onEdit={() => setEditingId(editingId === item.id ? null : item.id)} onCancel={() => { void cancel.mutateAsync({ id: item.id }); }} cancelPending={cancel.isPending} />)}
  </div>;
}
...
function RegistrationCard({ item, locale, editing, onEdit, onCancel, cancelPending }: { item: EventRegistrationListItem; locale: RegistrationLocale; editing: boolean; onEdit: () => void; onCancel: () => void; cancelPending: boolean }) {
  ...
  return <article className="border border-site-border bg-site-canvas p-5">
    ...
    {editing ? <form className="mt-4 grid gap-3 sm:grid-cols-3" onSubmit={(event) => { event.preventDefault(); void update.mutateAsync({ id: item.id, input: { locale, contact: { ...item.contact, first_name: firstName, last_name: lastName, phone }, participants: item.participants.map(({ id, first_name, last_name, dietary_restrictions, special_needs, additional_notes }) => ({ id, first_name, last_name, dietary_restrictions, special_needs, additional_notes })) } }).then(onEdit); }}><input value={firstName} onChange={(event) => setFirstName(event.target.value)} className="min-h-11 border border-site-border px-3 text-sm" aria-label={t("firstName")} /><input value={lastName} onChange={(event) => setLastName(event.target.value)} className="min-h-11 border border-site-border px-3 text-sm" aria-label={t("lastName")} /><input value={phone} onChange={(event) => setPhone(event.target.value)} className="min-h-11 border border-site-border px-3 text-sm" aria-label={t("phone")} /><button type="submit" disabled={update.isPending} className="min-h-11 bg-site-action px-4 text-sm font-semibold text-site-on-action sm:col-span-3">{t("saveChanges")}</button></form> : null}
