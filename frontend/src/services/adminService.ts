...
function createAdminService<T>(resource: string) {
    return {
        async getAll(params?: Record<string, string | number>): Promise<{ data: T[]; total: number }> {
        ...
        async update(id: number | string, data: Partial<T>): Promise<T> {
            const res = await api.put<ApiResponse<T>>(`/admin/${resource}/${id}`, data);
            return res.data.data!;
        },
...
export const galleryCategoryAdminService = {
    ...
    async create(data: Partial<GalleryCategory>): Promise<GalleryCategory> {
        const res = await api.post<ApiResponse<GalleryCategory>>('/admin/gallery/categories', data);
        return res.data.data!;
    },
    async update(id: number, data: Partial<GalleryCategory>): Promise<GalleryCategory> {
        const res = await api.put<ApiResponse<GalleryCategory>>(`/admin/gallery/categories/${id}`, data);
        return res.data.data!;
    },
};
...
export const donationCategoryAdminService = {
    ...
    async create(data: Partial<DonationCategory>): Promise<DonationCategory> {
        const res = await api.post<ApiResponse<DonationCategory>>('/admin/donation-categories', data);
        return res.data.data!;
    },
    async update(id: number, data: Partial<DonationCategory>): Promise<DonationCategory> {
        const res = await api.put<ApiResponse<DonationCategory>>(`/admin/donation-categories/${id}`, data);
        return res.data.data!;
    },
...
export const contactAdminService = {
    async getAll(params?: Record<string, string | number>): Promise<{ data: ContactInquiry[]; total: number }> {
    ...
    async updateStatus(id: number, status: string, replyMessage?: string): Promise<ContactInquiry> {
        const res = await api.put<ApiResponse<ContactInquiry>>(`/admin/contacts/${id}/status`, {
            status,
...
export const settingsAdminService = {
    ...
    async upsert(setting: Partial<Setting>): Promise<Setting> {
        const res = await api.post<ApiResponse<Setting>>('/admin/settings', setting);
        return res.data.data!;
    },
};
...
export const registrationAdminService = {
    async getAll(params?: Record<string, string | number>): Promise<{ data: Record<string, unknown>[]; total: number }> {
