    ...
    <section id="donate" className="border-t border-site-border bg-site-surface px-6 py-[clamp(4rem,9vw,8rem)] text-site-foreground sm:px-10 lg:px-[8vw]">
      <div>
        <motion.div
          ...
          className="mb-16 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]"
        >
          ...
          <div>
            ...
            <p className="mt-6 max-w-[65ch] text-lg leading-8 text-site-body">
              {t("description")}
            </p>
          </div>
        </motion.div>
        ...
        {contactQuery.isLoading ? (
          ...
          <div className="mx-auto max-w-4xl">
            <QueryErrorState
              ...
              isRetrying={contactQuery.isFetching}
            />
          </div>
        ) : !hasPaymentData ? (
          <div className="mx-auto max-w-4xl">
          ...
          <>
            <div className={`grid grid-cols-1 border-y border-site-border ${hasQr && hasBankTransfer ? "md:grid-cols-2" : "md:grid-cols-1"}`}>
              {hasQr && bank?.qr_image_url ? (
                <motion.div
                  ...
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.45, ease: easeOutSmooth, delay: reduceMotion ? 0 : 0.05 }}
                  viewport={{ once: true, margin: "-40px" }}
                  className="flex flex-col items-center bg-site-canvas p-8 text-center md:border-r md:border-site-border"
                >
                  ...
                  >
                    <PublicImage
                      ...
                      alt={t("qrAlt")}
                      fill
                      fallbackSrc={donationFallbackImage}
                      className="object-contain p-4 transition-transform duration-300 group-hover:scale-105"
                    />
                    <span className="absolute inset-x-4 bottom-4 bg-site-canvas/95 px-3 py-2 text-xs font-semibold text-site-foreground">
                      {t("clickToView")}
                    </span>
                  </button>
                  <button
              ...
              {hasBankTransfer && bank ? (
                <motion.div
                  initial={{ opacity: reduceMotion ? 1 : 0, y: reduceMotion ? 0 : 12 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.45, ease: easeOutSmooth, delay: reduceMotion ? 0 : 0.12 }}
                  viewport={{ once: true, margin: "-40px" }}
                  className="flex flex-col items-center bg-site-canvas p-8 text-center"
                >
                  ...
                  <div className="w-full space-y-4 border border-site-border bg-site-canvas p-6 text-left">
                    {bank.bank_name ? (
                      <div>
                        <p className="mb-1 text-xs font-semibold text-site-accent">{t("bankLabel")}</p>
                        <p className="font-medium text-site-foreground">{getLocalizedText(bank.bank_name, locale)}</p>
                      </div>
                    ) : null}
                    {bank.account_name ? (
                      <div>
                        <p className="mb-1 text-xs font-semibold text-site-accent">{t("accountNameLabel")}</p>
                        <p className="font-medium text-site-foreground">{getLocalizedText(bank.account_name, locale)}</p>
                      </div>
                    ) : null}
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      {bank.iban ? (
                        <div>
                          <p className="mb-1 text-xs font-semibold text-site-accent">{t("ibanLabel")}</p>
                          <p className="break-all font-mono font-medium text-site-foreground">{bank.iban}</p>
                        </div>
                      ) : null}
                      {bank.bic ? (
                        <div>
                          <p className="mb-1 text-xs font-semibold text-site-accent">{t("bicLabel")}</p>
                          <p className="font-mono font-medium text-site-foreground">{bank.bic}</p>
                        </div>
                      ) : null}
                    </div>
                    {bank.account_number ? (
                      <div>
                        <p className="mb-1 text-xs font-semibold text-site-accent">{t("accountNumberLabel")}</p>
            ...
            <div className="mt-8 flex flex-col gap-4 border-t border-site-border pt-8 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Link
                href="/donate/report"
                className="inline-flex min-h-11 shrink-0 items-center justify-center bg-site-action px-6 py-3 text-sm font-semibold text-site-on-action transition-colors hover:bg-site-action-hover focus-visible:outline-3 focus-visible:outline-offset-4 focus-visible:outline-site-focus"
              >
                {t("reportDonation")}
              </Link>
            </div>
      ...
      {showQrModal && bank?.qr_image_url ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setShowQrModal(false)}>
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-labelledby="donation-qr-title"
          ...
          >
            <h3 id="donation-qr-title" className="mb-4 text-center font-heading text-2xl font-bold text-site-foreground">
              {t("scanQr")}
            </h3>
            <div className="relative aspect-square overflow-hidden border border-site-border/15 bg-site-canvas">
              <PublicImage
                src={bank.qr_image_url}
                alt={t("qrAlt")}
                fill
